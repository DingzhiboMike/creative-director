...
**Language:** respond in the user's language (usually Russian). Method and framework names may stay in English.
...
1. Using `[[references/method-selection-matrix.md]]`, select 3 methods from different categories:
...
A qualitative check that informs the Scalability criterion in Axis 2 — use the answers to justify that score, don't count scalability twice.
...
**(b)** 5 passes completed → deliver the best with an honest assessment "here's where we stopped and why"; if nothing reached HumanKind 7, label them explicitly as below the bar (see the HumanKind rule)
...
**Rule:** below 7 = do not present as a recommendation. If stopping criterion (b) forces delivery, show such ideas only as "below the bar" with what's missing.
...
1. **NEVER** skip Phase 2 (insight). Without an insight, ideas are decoration. For quick Execution-level requests compress it to one insight line, but don't drop it
...
10. **Scores are self-assessments, not measurements.** Keep them conservative: when unsure, round down. A pass that changes the numbers but not the idea itself doesn't count — stop instead of polishing scores
...
→ Phase 1 (quick intake) → Phase 2 (compressed: one insight line) → Phase 3 (ideation, Execution-level) → brief evaluation → output
